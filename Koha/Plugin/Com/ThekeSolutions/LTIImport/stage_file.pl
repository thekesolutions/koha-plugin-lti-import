...
# Parts Copyright Theke Solutions 2019
#
...
sub process_batch {
    ...
    my $plugin = Koha::Plugin::Com::ThekeSolutions::LTIImport->new();

    my ( $errors, $marc_records );
    if( $format eq 'ISO2709' ) {
        ( $errors, $marc_records ) = $plugin->RecordsFromISO2709File( $params->{input_file}, $record_type, $params->{encoding} );
    } elsif( $format eq 'MARCXML' ) {
        ( $errors, $marc_records ) = $plugin->RecordsFromMARCXMLFile( $params->{input_file}, $params->{encoding} );
    }
